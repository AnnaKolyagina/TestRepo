...
function isValidPassword(password) {
  var symbolAmount = /^.{8,15}$/.test(password);
  var hasCapital = /[A-Z]/.test(password);
  var hasNumber = /\d/.test(password);
  var hasSpecChar = /[\^!@_$&*()\-\+]/.test(password);
  var isValid = symbolAmount && hasCapital && hasNumber && hasSpecChar;
  var matchDigits = password.match(/\d/g);
  var digits = matchDigits ? matchDigits.map(Number) : [];
  return { isValid: isValid, digits: digits };
}
...
// }
var fs_1 = require('fs');
var fileNames = (0, fs_1.readFileSync)('/Users/hannak/TestRepo/HW16/names.json', 'utf-8');
var newObject = JSON.parse(fileNames);
  ...
  users.forEach(function (user) {
    ...
    if (!emailReg.test(user.username)) {
      result[user.username] = {
        email: user.email,
        message: 'Hello '
          .concat((_a = user.name) !== null && _a !== void 0 ? _a : '', ' ')
          .concat((_b = user.last_name) !== null && _b !== void 0 ? _b : '', ' , please update your username "')
          .concat(user.username, '" to be a valid email to comply with our new policy.')
      };
    }
  });
  return result;
}
var checkResult = checkUsername(newObject);
